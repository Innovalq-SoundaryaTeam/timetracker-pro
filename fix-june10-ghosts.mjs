/**
 * fix-june10-ghosts.mjs
 * Cleans up ghost login/logout events from June 10, 2026 for every user
 * (same logic as fix-yesterday-ghosts.mjs, but for a fixed date):
 *  - Keeps only the FIRST 'login' of the day
 *  - Keeps only the FIRST 'logout' after that login
 *  - Deletes ANY event (of any type) timestamped AFTER that first logout
 *
 * Run with: node fix-june10-ghosts.mjs
 */
...
for (const userId of userIds) {
  ...
  //    everything (any type) after it, plus extra logouts
  if (logouts.length > 0) {
